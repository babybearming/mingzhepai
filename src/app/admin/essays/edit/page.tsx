...
function EssayEditForm() {
  ...
  useEffect(() => {
    ...
    fetch("/api/content?type=essay&slug=" + encodeURIComponent(slug))
      .then(res => {
  ...
  const handleSave = async (newSlug: string, frontmatter: any, content: string) => {
    const res = await fetch("/api/content", {
      ...
      body: JSON.stringify({ type: "essay", slug: newSlug, oldSlug: slug, frontmatter, content }),
    });
    if (!res.ok) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-foreground">编辑随笔</h1>
      </div>
